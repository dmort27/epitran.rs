...
impl M2MFstAligner {
   ...
   fn seq2fst(
            ...
            if self.grphm_eps {
               for l in 1..(self.max_phnm_len + 1) {
                  if j + l <= phoneme_seq.len() {
                     ...
                     fst.add_tr(istate as StateId, Tr::new(0, olabel, 1.0, ostate as StateId));
            ...
            if self.phnm_eps {
               for k in 1..(self.max_grphm_len + 1) {
                  if i + k <= grapheme_seq.len() {
                     ...
                     // symbtbl[0] is epsilon
                     fst.add_tr(istate as StateId, Tr::new(ilabel, 0, 1.0, ostate as StateId));
               ...
               for l in (1..(self.max_phnm_len + 1)) {
                  ...
                  if self.restrict && l > 1 && k > 1 {
                     ...
                     if (i + k) <= grapheme_seq.len() && (j + l) <= phoneme_seq.len() {
                        ...
                        fst.add_tr(istate as StateId, Tr::new(ilabel, olabel, 1.0, ostate as StateId));
      ...
      fst.set_start(0);
      fst.set_final(((grapheme_seq.len() + 1) * (phoneme_seq.len() + 1) - 1) as StateId, LogWeight::one());
   ...
   fn initialize(&mut self, num_unique_edges: f32) {
      ...
      for lattice in &self.lattices {
         let mut lattice2 = lattice.clone();
         for state_id in lattice.states_iter() {
            let mut trs = lattice2.tr_iter_mut(state_id).unwrap();
            for i in 0..trs.len() {
   ...
   fn expectation(&mut self) { // return necessary?
      ...
      for mut lattice in &self.lattices {
         ...
         if let Some(b0) = b0_opt {
            let mut lattice2 = lattice.clone();
            for state_id in lattice.states_iter() {
               let trs = lattice2.tr_iter_mut(state_id).unwrap();
               for i in 0..trs.len() {
                     ...
                     Some(tr) => {
                        ...
                        if let Some(a) = alpha_opt {
                           ...
                           if let Some(b) = beta_opt {
                              let v = a.times(tr.weight).unwrap().times(b).unwrap().divide(b0, DivideType::DivideAny).unwrap();
   ...
   fn reset_tr_weights(&mut self) { // have this return the max change in weight?
      let mut lattices2 = self.lattices.clone();
      for mut lattice in self.lattices {
         // .states_iter appears to be a method on VectorFst but lattices and lattices2 are Vec<VectorFst<LogWeight>>
         for state_id in lattices2.states_iter() {
